"""
[AI 로봇 제어 - 유료/고품질 음성 버전]

수정 내용:
1. gTTS(무료) 제거 -> OpenAI TTS(유료) 적용 : 훨씬 자연스러운 목소리
2. 릴레이 제거 -> 모터 3개 제어 유지
3. 한국어 음성 인식 및 대화 지원
"""
...
from openai import OpenAI  # [변경] 유료 고품질 음성 라이브러리
import cv2
...
def ai_model_response(u_input):
    ...
    """
    # 모델명은 사용 가능한 버전에 맞춰 수정 (예: gemini-1.5-flash)
...
# ------------------- Text-to-Speech Function (유료/고품질) -------------------

def text_to_speech(text):
    """
    [핵심 변경] OpenAI API를 사용하여 자연스러운 성우 목소리로 재생
    """
    # 목소리 옵션: alloy, echo, fable, onyx, nova, shimmer
    # 'nova'나 'shimmer'가 차분한 여성/중성 톤으로 비서 역할에 어울립니다.
    voice_option = "nova"
    ...
    try:
        ...
        # 생성된 오디오 데이터를 메모리에서 바로 재생
        play_audio(response.read())

    except Exception as e:
        print(f"OpenAI TTS 오류 (API 키를 확인하세요): {e}")
...
if __name__ == "__main__":
    ...
    # 시작 인사
    text_to_speech("안녕하세요. 고품질 음성 비서입니다. 무엇을 도와드릴까요?")
